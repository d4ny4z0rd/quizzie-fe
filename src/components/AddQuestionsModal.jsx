...
const AddQuestionsModal = ({ isOpen, onClose, onSubmit, title, type }) => {
  ...
  const handleCorrectOptionChange = (e) => {
    const updatedQuestions = [...questions];
    updatedQuestions[currentQuestionIndex].correctOption = parseInt(e.target.value);
    setQuestions(updatedQuestions);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setQuizLink(`http://localhost:5173/playquiz/${quizResponse?.quiz._id}`);
      ...
      <div className={styles.modalContent}>
        <div className={styles.questionNavigation}>
          {questions.map((_, index) => (
            <button
              key={index}
              className={index === currentQuestionIndex ? styles.activeQuestion : ""}
          ...
          <div>
            ...
            <label>
              <input
                type="radio"
                value="textImage"
                checked={questions[currentQuestionIndex].optionType === "textImage"}
                onChange={handleOptionTypeChange}
              />
              Text & Image URL
            </label>
        ...
        <div className={styles.timerContainer}>
          ...
          <div>
            <button
              ...
              className={questions[currentQuestionIndex].timer === 0 ? styles.activeTimer : ""}
            >
              OFF
            </button>
            <button
              onClick={() => handleTimerChange(5)}
              className={questions[currentQuestionIndex].timer === 5 ? styles.activeTimer : ""}
            >
              5 sec
            </button>
            <button
              onClick={() => handleTimerChange(10)}
              className={questions[currentQuestionIndex].timer === 10 ? styles.activeTimer : ""}
            >
              10 sec
            </button>
          </div>
